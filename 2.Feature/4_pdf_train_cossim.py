...
def main():
    ...
    for idx, row in train_df.iterrows():
        ...
        question_embeddings_cache[idx] = model.encode(question, convert_to_tensor=True, device=device)

    # PDF 문서 임베딩 캐싱
    pdf_embeddings_cache = {}
    for _, row in pdf_grouped.iterrows():
        pdf_text = row["문장"]
        pdf_embeddings_cache[row["파일명"]] = model.encode(pdf_text, convert_to_tensor=True, device=device)

    for idx, row in tqdm(train_df.iloc[2500:5000].iterrows()):
        ...
        # 🔹 질문 임베딩 가져오기
        question_embedding = question_embeddings_cache[idx]

        # 🔹 선택된 PDF 문서 임베딩 가져오기
        selected_pdf_embeddings = [pdf_embeddings_cache[filename] for filename in selected_pdf_filenames]

        # 🔹 유사도 계산
        similarities = util.pytorch_cos_sim(question_embedding, torch.stack(selected_pdf_embeddings))[0]
        ...
        # ✅ 결과 저장 (🚨 "유사한 PDF 전체 내용" 삭제)
        matched_results.append([question, keywords, best_match_pdf, best_score, top_matched_sentences])
    ...
    print("✅ 모든 질문 분석 완료!")
		...
		# ✅ **결과 저장** (🚨 "유사한 PDF 전체 내용" 제거됨)
		result_df = pd.DataFrame(matched_results, columns=["질문", "키워드", "PDF 파일명", "유사도", "PDF 내 관련 문장"])
